...
export const POPULATION_GOLDEN_LOCKS = Object.freeze({
  'population-a-initial-composite': Object.freeze({
    ...
    fitnessVectorVersion: 3, // v3: +per-member integrity observations (deliberate re-lock, 2026-07-24)
    ...
    // Deliberate re-lock 2026-07-24 (cause: intended encoding change -
    // fitness-vector v3 appends the five per-member integrity observations).
    // MEASURED before the re-lock: all 20 members remain integrity-clean, so
    // every per-member fitness/valid literal, the champion, and the champion
    // trace are UNCHANGED bit-exact - only the vector bytes moved (the
    // 34-byte observation block per member). v2 digest was 'a6d04f75'.
