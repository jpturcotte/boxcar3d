...
- **[V12]** RESOLVED (S1 PR) — the spring-model ruling, measured (the [V10] discipline repeated for the position motor): under **ForceBased** the isolated rig settles at target ± m·g/k EXACTLY (0.20500 @ 5 kg / 0.25000 @ 50 kg extension side, 0.18000 @ 20 kg compression side, k 20000, gravity 20) — stiffness is an honest N/m and damping changes decay, not equilibrium. **AccelerationBased settles 5 kg and 50 kg at the SAME coordinate (0.20100)** — mass-blind, its "stiffness" is not a spring rate — REJECTED (`S1_SPRING_MOTOR_MODEL_NAME = 'ForceBased'`, resolved symbolically). IN-CHAIN CAVEAT (recorded, not remediated): through the full chassis→prismatic→hub→revolute→wheel-on-ground chain, static sag inflates multiplicatively with damping × sprung:unsprung mass ratio (γ ≈ 0.33·c·dt/m_unsprung; exact at c=0 or heavy wheels) — solver convergence starvation, not a spring-law change; the chassis ADDITIONAL_SOLVER_ITERATIONS=4 policy is load-bearing (without it the same vehicle bottoms outright). Full matrix: `npm run probe:s1`.
