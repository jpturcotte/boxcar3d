...
export const POPULATION_GOLDEN_LOCKS = Object.freeze({
  'population-a-initial-composite': Object.freeze({
    ...
    fitnessVectorVersion: 3, // v3: +the five per-member integrity OBSERVATIONS
    ...
    // Deliberate re-lock 2026-07-24 (cause: intended encoding change — fitness
    // vector v3 persists the five integrity OBSERVATIONS). MEASURED before the
    // re-lock: every per-member fitness/valid literal below is UNCHANGED
    // bit-exact, as are the champion, the champion trace, the evaluation-spec
    // digest, the population-snapshot digest and the initialization digest.
    // ONLY the vector bytes moved, which is what a representation change looks
    // like — a policy change would have moved fitness values too.
    // Digest history: v1 'bded0d30' -> v2 'a6d04f75' -> v3 'fd4222eb'.
    //
    // THE OBSERVATIONS ARE NOW PART OF THE LOCK, necessarily: under v2 a clean
    // status alone determined the bytes, and it no longer does. They are also
    // the first committed record of what this fixture's integrity margins
    // actually are — all 20 members alert-free, peak body speed 0.9696 to
    // 4.8661 m/s, i.e. 5.14x below the 25 m/s alert line at the worst member.
    // MEASURED VALUES, never thresholds: this file must not assert a margin.
